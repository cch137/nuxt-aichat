...
export default defineEventHandler(async (event) => {
  ...
  if (!rateLimiterBundler.check(ip)) {
    return await new Promise((r) =>
      setTimeout(() => r({ error: rateLimiterBundler.getHint(ip) }), 10000)
    );
  }
  const now = Date.now();
  ...
  const neededAuthlvl =
    models.find((m) => m.value === model)?.permissionLevel || 0;
  ...
  if (
    (analyzeLanguages(
      (messages as OpenAIMessage[]).map((m) => m.content).join("")
    )?.ru || 0) > 0.25
  ) {
  ...
  try {
    const lastQuestion =
      (messages as OpenAIMessage[]).findLast((i) => i.role === "user")
        ?.content || "";
    ...
    const croppedMessages = (() => {
      ...
      const maxTokens =
        model === "gpt4"
          ...
          : 4000;
      while (
        estimateTokens(model, JSON.stringify(_messages)) > maxTokens &&
        _messages.length > 1
      ) {
        _messages.shift();
      }
