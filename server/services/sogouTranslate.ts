...
const sogouCrypto = (from: string, to: string, text: string) =>
  md5(`${from}${to}${text}${secretCode}`);
...
const init = () =>
  new Promise((resolve, reject) => {
    ...
    session
      .get("https://fanyi.sogou.com/text")
      .then((res) => {
        const { data } = res;
...
const translate = (
  ...
  new Promise((resolve, reject) => {
    ...
    session
      .post("https://fanyi.sogou.com/api/transpc/text/result", {
        text,
        from,
        to,
        client: "pc",
      ...
      .then((res) => {
        ...
        const apiResponse: any = {
          ...
          text: (data.sentencesData.trans_result || [
            { trans_text: data.translate.dit },
          ])[0].trans_text,
        };
        if (+rawData) {
...
export { init, translate, translateZh2En };
