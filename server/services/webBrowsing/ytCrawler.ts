...
function parseVideoInfo(html: string) {
  ...
  try {
    ...
    return {
      ...
      lengthSeconds:
        "lengthSeconds" in data ? Number(data?.lengthSeconds) : undefined,
      ...
      isUnlisted:
        "externalChannelId" in data
          ? Boolean(data?.externalChannelId)
          : undefined,
    };
  } catch {
    return null;
...
async function getYouTubeCaptions(
  html: string,
  lang?: string,
  options = { ignoreNotSupportedLanguage: true }
) {
  ...
  const captionTracks = JSON.parse(`${match1}}`).captionTracks as {
    baseUrl: string;
  ...
  const { baseUrl } =
    captionTracks.find((c) => c.languageCode === lang) ||
    captionTracks.find((c) => c.isTranslatable) ||
    {};
  if (!baseUrl) {
  ...
  const captionsSourceUrl = `${url.origin}${url.pathname}${decodeURIComponent(
    qs.stringify(apiOptions)
  )}`;
  const captions = (
    await axios.get(captionsSourceUrl, { validateStatus: (_) => true })
  ).data?.events as {
    tStartMs: number;
    dDurationMs: number;
    segs: [{ utf8: string }];
