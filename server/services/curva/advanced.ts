...
const gpt4ScrapeAndSummary = async (client: MindsDBClient, question: string, url: string, userTimeZone = 0, delay = 0) => {
  try {
...
export default async function (client: MindsDBClient, question: string, context = '', userTimeZone = 0) {
  try {
    let i = 0
    const question1 = useParseUrlsAndQueries(question, userTimeZone)
    const answer1 = (await makeRequest(client, 'gpt4_summarizer', question1))?.answer as string
    ...
    const _pages1 = urls.map((url) => gpt4ScrapeAndSummary(client, question, url, userTimeZone, i += 1000))
    ...
    const answer2 = (await makeRequest(client, 'gpt4_summarizer', question2))?.answer as string
    ...
    const _pages2 = selectedSiteUrls.map((url) => gpt4ScrapeAndSummary(client, question, url, userTimeZone, i += 1000))
