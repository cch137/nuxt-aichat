...
function getSearchPhrasesPrompt(
  question: string,
  context: string
): OpenAIMessage[] {
  return [
...
class Gpt4FgaWebChatbot {
  ...
  async ask(
    ...
    const searchPhrasesRes = await this.core.ask(
      getSearchPhrasesPrompt(question, context),
      { model: "gpt-3.5-turbo", temperature: 0 }
    );
    const queries: string[] =
      parseObjectFromText(searchPhrasesRes?.answer || "{}")?.queries || [];
    return {
