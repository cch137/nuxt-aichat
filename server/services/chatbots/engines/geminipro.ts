...
const geminiPro = (() => {
  ...
  const ask = async (
    newMessage = "Hi",
    _history: InputContent[] = [],
    streamId?: string
  ) => {
    const stream =
      (streamId === undefined ? null : streamManager.get(streamId)) ||
      streamManager.create();
    const model = genAI.getGenerativeModel({ model: "gemini-pro" });
    const history: InputContent[] = [];
    _history = _history.map((msg) => ({
      role: msg.role,
      ...
      while (true) {
        ...
        if (promptFeedback?.blockReason) {
          globalError = new Error(
            `Model refused to respond. (Ratings: ${promptFeedback.safetyRatings
              .map((r) => `${r.probability} ${r.category}`)
              .join(", ")})`
          );
          throw globalError;
        }
...
class HackedGeminiProChatbot {
  ...
  async ask(
    ...
    try {
      const stream = await geminiPro.ask(
        question,
        messages.map((m) => ({
          parts: m.content,
