...
function getSelectSql(modelName: string, question: string, context = "") {
  ...
  return `SELECT answer FROM mindsdb.${modelName} WHERE question = ${wrapPromptTextParam(
    question
...
class MindsDBClient {
  ...
  constructor(email: string, password: string, connectMethod?: "SQL" | "WEB") {
  ...
  async askGPT(modelName: string, question = "", context = "") {
    const client =
      containsDoubleDash(question) || containsDoubleDash(context || "")
        ? this.webClient
        : this.client;
...
class MindsDBSqlClient extends _Client {
  ...
  login() {
    const sequelize = new Sequelize("mindsdb", this.email, this.password, {
  ...
  async askGPT(modelName: string, question = "Hi", context = "") {
    try {
      ...
      return {
        answer: "",
        error: "The source did not return a valid response.",
      };
    } catch (err) {
      console.error(`${err}`);
      return {
        answer: "",
        error: (err as any)?.original?.sqlMessage as string,
      };
    }
  }
}
