...
const getUser = async (
  uid: string
): Promise<{ username: string; email: string; authlvl: number } | null> => {
  return await userCollection.findOne(
    { uid },
    { _id: 0, username: 1, email: 1, authlvl: 1 }
  );
};
