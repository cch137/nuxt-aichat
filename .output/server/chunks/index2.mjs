import { m as message, l as libExports } from './index3.mjs';
...
const chatbotUsageRecord$1 = chatbotUsageRecord;
  ...
  async ask(ip, user, conv, model = "gpt4", temperature = 0.5, messages = [], tz = 0, _id) {
    ...
    try {
      const engine = await (async () => {
        ...
        return ["gpt3", "gpt4", "gpt-web"].includes(model) ? new Engine(await coreCollection$1.get(getRandomToken())) : new Engine(await coreCollection$1.get(freeGptAsiaToken));
      })();
      const t0 = Date.now();
