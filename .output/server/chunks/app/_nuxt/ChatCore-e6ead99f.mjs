...
import { f as useUniCookie, g as useNuxtApp, h as useLocale, u as useState, n as navigateTo, d as useId, t as throwError, b as useNamespace } from '../server.mjs';
import { ref, useSSRContext, reactive, watch, nextTick, createApp, toRefs, isRef, isVNode, defineComponent, h as h$1, Transition, withCtx, withDirectives, createVNode, vShow, render, computed, resolveComponent, openBlock, createBlock, createElementVNode, normalizeClass, normalizeStyle, withModifiers, createElementBlock, resolveDynamicComponent, createCommentVNode, toDisplayString, withKeys, renderSlot, createTextVNode, onScopeDispose, provide, unref } from 'vue';
import { u as useTitle } from './useTitle-dc6f5342.mjs';
import sha3 from 'crypto-js/sha3.js';
import md5 from 'crypto-js/md5.js';
import { b as buildProps, d as definePropType, E as ElMessage, g as getStyle, a as addClass, r as removeClass, u as useGlobalComponentSettings, i as isUndefined, _ as _export_sfc, c as isElement, e as ElButton, f as ElInput, h as ElIcon, T as TypeComponents, j as TypeComponentsMap, k as componentSizes, l as hasClass, m as EVENT_CODE } from './index-e055d37d.mjs';
...
var ElFocusTrap = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["render", _sfc_render$1], ["__file", "/home/runner/work/element-plus/element-plus/packages/components/focus-trap/src/focus-trap.vue"]]);
...
const CONTEXT_MAX_LENGTH = 3e3;
const getContext = () => {
  if (!contextMode.value) {
    return "";
  }
  const contexts = messages.value.filter((msg) => msg.done).map((message) => {
...
const getQuestionSuggestions = async function(question) {
  return await $fetch("/api/chat/suggestions", { method: "POST", body: { question } });
};
...
const fetchHistory = (conv) => {
  return new Promise((resolve, reject) => {
    ...
    $fetch("/api/chat/history", { method: "POST", body: { id: conv } }).then(async (fetched) => {
      const archived = fetched;
      ...
      getQuestionSuggestions(lastMessage.Q).then((more) => {
...
const getWebBrowsing = () => {
  return webBrowsingMode.value;
};
const getHashType = () => {
  return [77, 68, 53].map((c) => String.fromCharCode(c)).join("");
};
const createHeaders = (message, context, t) => {
  const hash = createHash(`${message}${context}`, getHashType(), t);
  const timestamp = str(t);
  return { hash, timestamp };
};
const createBody = (message, model2, web, t, tz, regenerateId) => {
  var _a, _b;
  let conv = (_b = (_a = useNuxtApp()._route) == null ? void 0 : _a.params) == null ? void 0 : _b.conv;
  if (!conv) {
    conv = random.base64(8);
    conversations.value.push({ id: conv, name: void 0 });
    navigateTo(`/c/${conv}`);
  }
  return { conv, context: getContext(), prompt: message, model: model2, web, t, tz, id: regenerateId };
};
const createRequest = (message, regenerateId) => {
...
function useChat() {
  ...
  const initPage = async (conv, skipHistoryFetching = false) => {
  ...
  const goToChat = async (conv, force = false, skipHistoryFetching = false) => {
    const currentConvId = getCurrentConvId();
    if (useDevice().isMobileScreen) {
  ...
  const sendMessage = (forceMessage, regenerateId) => {
    ...
    createRequest(messageText, regenerateId).then((res) => {
      const isAtBottom = getScrollTop() >= document.body.clientHeight;
      const id = res.id;
      ...
      const dt = res.dt;
      const isQuestionComplete = "complete" in res ? res.complete : true;
  ...
  const renameConversation = () => {
    ElMessageBox.prompt(_t("message.renameConvHint"), _t("message.setting"), {
      ...
      $fetch("/api/chat/conv", { method: "PUT", body: { id: getCurrentConvId(), name } }).then(async () => {
        ElMessage({
          type: "success",
...
const _sfc_main = {
  ...
  setup(__props) {
    const { messages: messages2, openSidebar: openSidebar2, sendMessage, deleteMessage, regenerateMessage } = useChat();
...
//# sourceMappingURL=ChatCore-e6ead99f.mjs.map
