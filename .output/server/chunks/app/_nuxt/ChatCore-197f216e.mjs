...
const getContext = () => {
  ...
  const contexts = messages.value.filter((msg) => msg.done).map((msg) => `Question: ${msg.Q}
Answer: ${msg.A}`);
...
const initPage = (conv, skipHistoryFetching = false) => {
  if (!skipHistoryFetching) {
    ...
    Promise.all([
      ...
      useScrollToBottom();
      if (loading !== null) {
        useScrollToBottom(500).finally(() => loading.close());
      }
    });
  }
};
...
const createBody = (message, model2, web, t, tz) => {
  var _a, _b;
  ...
  return { conv, context: getContext(), prompt: message, model: model2, web, t, tz };
};
...
function useChat() {
  ...
  const sendMessage = (forceMessage) => {
    ...
    useScrollToBottom();
    createRequest(messageText).then((res) => {
      const id = res.id;
    ...
    getQuestionSuggestions(messageText).then((more) => {
      const isAtBottom = getScrollTop() >= document.body.clientHeight;
      message.more = more;
      if (isAtBottom) {
        useScrollToBottom();
...
const _sfc_main = {
  ...
  setup(__props) {
    const { messages: messages2, openSidebar: openSidebar2, sendMessage } = useChat();
...
//# sourceMappingURL=ChatCore-197f216e.mjs.map
