...
async function useScrollToBottom() {
...
const predictMoreQuestions = async function(question) {
  return await $fetch("/api/more", { method: "POST", body: { question } });
};
...
const fetchHistory = (conv) => {
  return new Promise((resolve, reject) => {
    ...
    $fetch("/api/history", { method: "POST", body: { id: conv } }).then(async (fetched) => {
      ...
      resolve(true);
      const lastQuestion = messages.value.at(-2);
      const lastAnswer = messages.value.at(-1);
...
const createHeaders = (message, context2, t) => {
  const hash = createHash(`${message}${context2}`, getHashType(), t);
...
const createBody = (message, model2, web, t, tz) => {
  ...
  return { conv, context: context.get(), prompt: message, model: model2, web, t, tz };
};
...
function useChat() {
  ...
  const sendMessage = (forceMessage) => {
    ...
    useScrollToBottom();
    createRequest(message).then((res) => {
      ...
      answerMessage.t = /* @__PURE__ */ new Date();
    });
    predictMoreQuestions(message).then((more) => {
